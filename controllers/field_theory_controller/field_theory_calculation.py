...
def distance_target(box_position, BOX_D, target_position):
    A_position = [box_position[0] + BOX_D / 2 * np.cos(box_position[2]),#center position
                  box_position[1] - BOX_D / 2 * np.sin(box_position[2])]
    B_position = [box_position[0] + BOX_D / 2 * np.cos(math.pi / 2 + box_position[2]),
                  box_position[1] - BOX_D / 2 * np.sin(math.pi / 2 + box_position[2])]
    C_position = [box_position[0] + BOX_D / 2 * np.cos(math.pi + box_position[2]),
                  box_position[1] - BOX_D / 2 * np.sin(math.pi + box_position[2])]
    D_position = [box_position[0] + BOX_D / 2 * np.cos(3 * math.pi / 2 + box_position[2]),
                  box_position[1] - BOX_D / 2 * np.sin(3 * math.pi / 2 + box_position[2])]
    box_p = [box_position[0],box_position[1]]
