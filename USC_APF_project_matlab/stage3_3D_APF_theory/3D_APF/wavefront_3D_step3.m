function cost = wavefront_3D_step3(goal, logitMap)
...
while size(open,1) ~= 0
  ...
  for k=1:size(adjacent,1)
    ...
    % Make sure the adjacent cell is in S skeleton 
    if occ(adj(1), adj(2), adj(3)) < 0.5
      continue
    end
    ...
    cost(adj(1), adj(2), adj(3)) = cost(open(1,1), open(1,2), open(1,3)) + 0.2;
