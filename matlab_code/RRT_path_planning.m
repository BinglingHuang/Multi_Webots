...
for i = 1:maxattempts
    ...
    while(t<1)
        ...
        if abs(Qnear(3)-Qrand(3)) <= 90
            theta_temp = (1-t)*Qnear(3)+(t)*Qrand(3);
        else
            n_theta = sort([Qnear(3) Qrand(3)]);
            theta_temp = (1-t)*n_theta(1)+(t)*(n_theta(2)-180);
...
path = path(end:-1:1,:)
for i = 1:length(path)
    box_map(path(i,:), W, b_length, b_width);
end
